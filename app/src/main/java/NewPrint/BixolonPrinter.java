...
public class BixolonPrinter implements ErrorListener, OutputCompleteListener, StatusUpdateListener, DirectIOListener, DataListener {
    ...
    public boolean printerOpen(int portType, String logicalName, String address, boolean isAsyncMode) {
        Log.e("dtails","portType"+portType+" logicalName "+logicalName+" address"+address+" isAsyncMode"+isAsyncMode);
        if (setTargetDevice(portType, logicalName, BXLConfigLoader.DEVICE_CATEGORY_POS_PRINTER, address)) {

            Log.e("details",portType+" ^ "+address+" ^ "+logicalName+" ^ "+isAsyncMode);
            try {
    ...
    public boolean printPDF(PdfPage bitmap, int width, int alignment, int brightness) {
        boolean ret = true;
        ...
        try {
            ...
            posPrinter.printPDF(buffer.getInt(0), String.valueOf(bitmap), width, alignment, 0, 0);
        } catch (JposException e) {
            e.printStackTrace();
    ...
    public boolean cutPaper() {
        try {
            ...
            String cutPaper = EscapeSequence.ESCAPE_CHARACTERS + String.format("%dfP", 50); // Feed Partial Cut
